...
function [all] = twordsa_lex_dp(p, sigma, type, numSimulations, replications, theta_0)
...
%    Hbar=1.05*2*B'*B;
  ...
  for k=0:(N/(2*3^p))-1    %use of N-avg is to account for avg used in setting lossold 
    ak = a1/(k+1+A1)^alpha1;
...
%  
for k=0:((numSimulations-N)/(3*3^p)) - 1
    a_k=a1/(k+1)^alpha2;
%     c_k=c2/((k+1)*3^p + m)^gamma1;
    ghatinput=0;
    Hhatinput=0;
